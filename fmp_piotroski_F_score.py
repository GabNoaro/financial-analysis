...
for i, component in enumerate(component_names, 1):
    plt.subplot(3, 3, i)  # 3x3 grid with adjusted height
    sns.lineplot(x='Date/Period', y=component, hue='Symbol', data=components_data, marker='o', linestyle='-')
    plt.title(f'{component} Over Time')
    plt.xlabel('Date/Period')
    #plt.ylabel(component)
    plt.ylabel('') # Empty string to remove the y-axis label
    plt.xticks(rotation=60, ha='right')  # Rotate x-axis labels by X degrees
    plt.yticks([0, 1])  # Display only 0 and 1 on y-axis
    plt.legend()
    plt.tight_layout(pad=5.0)  # Adjust the padding between subplots

# Adjust the aspect ratio to shorten the y-axis
plt.subplots_adjust(wspace=0.1, hspace=0.9)  # Adjust the space between subplots
# Set aspect ratio to make y-axis shorter
plt.gca().set_aspect(0.1)  # Adjust the value to your preference
plt.show()